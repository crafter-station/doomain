...
export const commandSchemas: CommandSchema[] = [
  {
    ...
    agentHint:
      'Use this for non-Vercel infrastructure such as a VPS or load balancer. Run without --dry-run unless the user explicitly requests a preview; conflicts fail safely unless --force is explicit.',
    agentInstructions: [
  ...
  },
  {
    ...
    flags: [
      ...
      },
      {
        ...
        type: 'string',
        description:
          'Target domain or base zone, for example app.example.com or example.com. Optional when DOOMAIN_DOMAIN or a default domain is configured.',
      },
      ...
      },
      {
        ...
        type: 'boolean',
        description:
          'Preview changes without writing. Intended for human previews; agents should not use this unless explicitly asked.',
      },
      {
        name: 'force',
        type: 'boolean',
        description:
          'Move existing Vercel project domains and overwrite conflicting DNS records. Interactive DNS override confirmation does not move Vercel aliases; pass --force for that.',
      },
      {
        name: 'wait',
        type: 'boolean',
  ...
  },
  {
    ...
    description:
      'Create the first Clerk production instance with its primary domain and configure returned DNS records.',
    examples: [
