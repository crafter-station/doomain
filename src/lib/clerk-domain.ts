...
function waitForStatus(
  ...
  timeoutSeconds: number,
  progress?: (message: string) => void,
): DoomainEffect<ClerkDomainStatus> {
  return Effect.gen(function* () {
    const trigger = yield* client.triggerDomainDnsCheck(app, domainId).pipe(Effect.either)
    if (trigger._tag === 'Left') {
      const conflict = JSON.stringify(trigger.left.details).toLowerCase().includes('conflict')
      if (!conflict) return yield* Effect.fail(trigger.left)
    }
    ...
    while (Date.now() <= deadline) {
      ...
      status = yield* client.getDomainStatus(app, domainId)
...
export function addClerkProductionDomain(input: AddClerkDomainInput): DoomainEffect<AddClerkDomainResult> {
  return Effect.gen(function* () {
    const domain = yield* trySync(() => normalizeDomain(input.domain ?? ''), 'INVALID_INPUT')
    const config = yield* resolveClerkPlatformConfig(input.app)
    const client = createClerkPlatformClient(config)
    input.progress?.('Checking Clerk application')
    const application = yield* client.fetchApplication(config.appId)
    const production = application.instances.find((instance) => instance.environment_type === 'production')
    if (production) return yield* Effect.fail(productionExistsError(config.appId, production.instance_id))
    ...
    const created = yield* client.createProductionInstance(config.appId, domain, development.instance_id)
    ...
    const records = yield* trySync(
      () => recordsFromTargets(resolved.provider, resolved.target.zoneDomain, clerkDomain.cname_targets ?? []),
      'INVALID_INPUT',
    )
    if (records.length === 0) {
      return yield* Effect.fail(
    ...
    let dnsPlan = yield* provider.planChanges(zone, records, { force: forceDns })
    ...
    const status = shouldWait
      ? yield* waitForStatus(client, config.appId, clerkDomain.id, input.timeoutSeconds ?? 300, input.progress)
      : undefined
