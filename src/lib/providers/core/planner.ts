...
export function planDnsChanges(input: {
  ...
  for (const record of input.desired) {
    ...
    if (record.type === 'TXT') {
      changes.push(
        exact ? { action: 'skip', existing: exact, reason: 'already_exists', record } : { action: 'create', record },
      )
      continue
    }

    const sameTyped = input.existing.filter((existing) => sameSlot(existing, record) && !sameDnsValue(existing, record))
    const cnameConflicts = input.existing.filter(
      (existing) => cnameSlotConflict(existing, record) && !sameSlot(existing, record),
    )
    ...
    changes.push(
      ...
      ...remainingSameTyped.map((existing) => ({
