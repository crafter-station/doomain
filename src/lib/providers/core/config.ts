...
function credentialFromSavedConfig(
...
export function getProviderCredentials(
  ...
  const credentials =
    account === DEFAULT_PROVIDER_ACCOUNT ? current?.credentials : current?.accounts?.[account]?.credentials
...
export function providerAccountHasCredentials(
  config: DoomainConfig,
  providerId: string,
  accountInput?: string,
): boolean {
  ...
  return (
    hasCredentials(current.credentials) ||
...
export function isProviderAccountConfigured(
  ...
  return definition.credentials.every(
    (credential) =>
      credential.required === false || Boolean(getProviderCredential(config, definition.id, credential, opts)),
  )
}

export function listConfiguredProviderAccounts(
  config: DoomainConfig,
  definition: DnsProviderDefinition,
): ProviderAccountRef[] {
