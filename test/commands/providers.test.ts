...
describe('providers', () => {
  ...
  it('detects saved provider account credentials', () => {
      ...
      providerAccountHasCredentials(
        {
          providers: {
            spaceship: {
              accounts: { work: { credentials: { apiKey: 'work_key' } } },
    ...
    expect(
      providerAccountHasCredentials(
        { providers: { spaceship: { apiKey: 'legacy_key', apiSecret: 'legacy_secret' } } },
        'spaceship',
      ),
    ).to.equal(true)
    expect(
  ...
  it('reports provider configuration status without verification', async () => {
    ...
    expect(
      result.data.providers.map((provider) => ({ configured: provider.configured, id: provider.id })),
    ).to.deep.equal([
  ...
  it('disconnects a saved provider without removing unrelated config', async () => {
    ...
    expect(config.providers?.cloudflare).to.deep.equal({
      credentials: { accountId: 'account_123', apiToken: 'cloudflare_token' },
    })
  ...
  it('connects a named provider account without overwriting the default account', async () => {
    ...
    expect(config.providers?.spaceship?.credentials).to.deep.equal({
      apiKey: 'default_key',
      apiSecret: 'default_secret',
    })
    expect(config.providers?.spaceship?.accounts?.work?.credentials).to.deep.equal({
      apiKey: 'work_key',
      apiSecret: 'work_secret',
    })
  })
