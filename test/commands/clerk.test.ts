  ...
  it('creates first production, writes DNS-only CNAMEs, and checks Clerk status', async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      if (
        ...
        url.pathname === '/client/v4/zones/zone_1/dns_records' &&
        !init?.method
      ) {
      ...
      if (url.hostname === 'api.clerk.com' && url.pathname.endsWith('/status')) {
        return jsonResponse({
          dns: { status: 'complete' },
          mail: { required: true, status: 'complete' },
          ssl: { required: true, status: 'complete' },
          status: 'complete',
        })
      }
    ...
    const result = JSON.parse(stdout) as {
      data: {
        clerk: { productionInstanceCreated: boolean; verified: boolean }
        nextSteps: string[]
        records: Array<{ name: string; proxied: boolean }>
      }
      ok: boolean
    }
    ...
    expect(result.data.records).to.deep.include({
      name: 'clerk',
      proxied: false,
      ttl: 300,
      type: 'CNAME',
