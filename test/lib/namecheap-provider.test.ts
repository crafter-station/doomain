...
describe('namecheap provider', () => {
  ...
  it('lists DNS host records from Namecheap XML', async () => {
    ...
    expect(records).to.deep.equal([
      {
        id: '1',
        metadata: { namecheap: { Address: '1.1.1.1', HostId: '1', Name: '@', TTL: '1800', Type: 'A' } },
        name: '@',
        ttl: 1800,
        type: 'A',
        value: '1.1.1.1',
      },
      {
        id: '2',
        metadata: {
          namecheap: { Address: 'mail.example.com', HostId: '2', MXPref: '10', Name: 'mail', TTL: '1800', Type: 'MX' },
        },
        name: 'mail',
        priority: 10,
        ttl: 1800,
  ...
  it('preserves unrelated records when applying setHosts', async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      return {
        ok: true,
        status: 200,
        text: async () => (command === 'namecheap.domains.dns.setHosts' ? okXml : hostsXml),
      } as Response
    }) as typeof fetch
    ...
    const setHostsRequest = requests.find(
      (request) => namecheapParams(request.input, request.init).get('Command') === 'namecheap.domains.dns.setHosts',
    )!
  ...
  it('throws when setHosts does not confirm success', async () => {
    globalThis.fetch = (async (input, init) => {
      ...
      return {
        ok: true,
        status: 200,
        text: async () => (command === 'namecheap.domains.dns.setHosts' ? failedSetHostsXml : hostsXml),
      } as Response
    }) as typeof fetch
  ...
  it('does not save invalid provider credentials when verification fails', async () => {
    ...
    globalThis.fetch = (async () =>
      ({
        ...
        status: 200,
        text: async () =>
          '<ApiResponse Status="ERROR"><Errors><Error>API key is invalid</Error></Errors></ApiResponse>',
      }) as Response) as typeof fetch

    try {
      const { runCommand } = await import('@oclif/test')
      const result = await runCommand(
        'providers connect namecheap --credential apiUser=user --credential apiKey=bad --credential clientIp=127.0.0.1 --json',
      )
