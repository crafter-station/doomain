...
describe('clerk platform client', () => {
  ...
  it('creates a production instance through the Clerk Platform API', async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      request = init
      return new Response(JSON.stringify({id: 'ins_prod', environment_type: 'production', active_domain: null, publishable_key: 'pk_live'}), {
        status: 201,
      })
    }) as typeof fetch

    await createClerkPlatformClient({platformApiKey: 'ak_test'}).createProductionInstance('app_123', 'example.com', 'ins_dev')

    expect(request?.method).to.equal('POST')
    expect(new Headers(request?.headers).get('Authorization')).to.equal('Bearer ak_test')
    expect(JSON.parse(String(request?.body))).to.deep.equal({clone_instance_id: 'ins_dev', domain: 'example.com', environment_type: 'production'})
  })

  it('returns an actionable Clerk authorization error', async () => {
    globalThis.fetch = (async () => new Response(JSON.stringify({errors: [{message: 'Forbidden'}]}), {status: 403})) as typeof fetch
